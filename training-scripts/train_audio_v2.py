#!/usr/bin/env python3
"""Audio deepfake detector training v2.
Uses Wav2Vec2 backbone fine-tuned for real vs synthetic audio classification.
Key changes from v1: more diverse training data, more epochs, adjusted augmentation.
"""
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # Random reverb simulation (simple convolution with decay)
            if random.random() < 0.1:
                delay = random.randint(800, 3200)  # 50-200ms at 16kHz
...
def package_model(model, output_dir, swap_labels=False):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "wav2vec2-audio-deepfake-detector-v2"
version: "2.0.0"
modality: "audio"
...
def main():
    ...
    OUTPUT_DIR = '/root/audio_detector_v2'
    SEED = 42
    EPOCHS = 25
    BATCH_SIZE = 16
    LR = 1e-5
    ...
    # Soft balance: cap majority class at 1.5x minority
    ...
    # Freeze feature extractor only, train transformer + classifier
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_audio_model_v2.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_audio_model_v2.pt', weights_only=True))
