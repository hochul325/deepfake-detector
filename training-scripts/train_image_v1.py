#!/usr/bin/env python3
"""Image deepfake detector training v1.
Uses CLIP ViT-B/16 backbone, same approach as video model.
Input: [B, 3, H, W] float32 RGB 0-255
Output: [B, 2] logits (class 0=real, class 1=synthetic)
"""
...
class ImageDeepfakeDetector(nn.Module):
    ...
    def _freeze_backbone(self):
        ...
        # Unfreeze last 6 blocks + all norms
...
def load_parquet_with_label(parquet_path, label, max_samples=5000):
    """Load images from parquet. label is applied to all samples."""
    samples = []
    try:
...
def load_parquet_with_label_col(parquet_path, max_samples=10000):
    """Load images from parquet that has a 'label' column."""
    samples = []
    try:
        table = pq.read_table(parquet_path)
        df = table.to_pandas()
...
def package_model(model, output_dir):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "clip-vit-image-deepfake-detector-v1"
version: "1.0.0"
modality: "image"
...
def main():
    ...
    OUTPUT_DIR = '/root/image_detector_v1'
    SEED = 42
    EPOCHS = 25
    BATCH_SIZE = 64  # Larger batch for single images (no temporal dim)
    LR = 8e-6
    VAL_SPLIT = 0.15
    ...
    # Load parquets with explicit labels first
    for f in sorted(data_dir.glob('*.parquet')):
        ...
        if has_label:
            samples = load_parquet_with_label_col(str(f), max_samples=5000)
        ...
        if any(k in name for k in real_keywords):
            ...
            samples = load_parquet_with_label(str(f), label=1, max_samples=5000)
            all_samples.extend(samples)
        else:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_image_model_v1.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_image_model_v1.pt', weights_only=True))
