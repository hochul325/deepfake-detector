#!/usr/bin/env python3
"""Image deepfake detector training v3b.
Same as v3 but different seed (123) for ensemble diversity.
"""
...
def package_model(model, output_dir):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "clip-vit-image-deepfake-detector-v3b"
version: "3.1.0"
modality: "image"
...
def main():
    ...
    OUTPUT_DIR = '/root/image_detector_v3b'
    SEED = 123
    EPOCHS = 30
    BATCH_SIZE = 64
    LR = 8e-6
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_image_model_v3b.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_image_model_v3b.pt', weights_only=True))
