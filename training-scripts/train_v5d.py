#!/usr/bin/env python3
"""Video deepfake detector training v5d.
Changes from v5b:
- Seed=123 (different init/data ordering)
- Label smoothing 0.10 (was 0.08) - reduces overconfidence
- Add RandomErasing(p=0.15) to augmentation
- Train for 30 epochs (was 25) with patience 10 (was 8)
- Slightly stronger color jitter
- Package with non-TTA model.py (TTA found to be worse)
"""
...
def package_model(model, output_dir):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "clip-vit-video-deepfake-detector-v5d"
version: "5.4.0"
modality: "video"
...
""")
    ...
    # Non-TTA model (found to perform better than TTA)
...
def main():
    ...
    OUTPUT_DIR = '/root/video_detector_v5d'
    SEED = 123
    EPOCHS = 30
    BATCH_SIZE = 32
    LR = 8e-6
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_model_v5d.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_model_v5d.pt', weights_only=True))
