#!/usr/bin/env python3
"""Video deepfake detector training script v5.
Key changes from v4:
- Focal loss instead of CE (focuses on hard examples)
- Higher label smoothing (0.1)
- More aggressive mixup (0.4 alpha, 50% chance)
- CutMix augmentation for additional robustness
- No class balancing cap - use WeightedRandomSampler properly
- Longer training (25 epochs) with patience 8
- Test-time augmentation in model.py (horizontal flip)
- Lower backbone LR (5e-6 vs 1e-5)
...
def train_epoch(model, loader, optimizer, scheduler, device, epoch,
                mixup_alpha=0.4, criterion=None):
    model.train()
...
""")
    ...
    # Model with test-time augmentation (horizontal flip averaging)
...
class VideoDeepfakeDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        Uses TTA: average logits from original + horizontally flipped.
        """
        if x.dim() == 4:
            x = x.unsqueeze(1)
        B, T = x.shape[:2]
        ...
        # Average features before classifier
...
def main():
    ...
    EPOCHS = 25
    BATCH_SIZE = 32
    LR = 5e-6  # Lower backbone LR for more stable training
    VAL_SPLIT = 0.15
    ...
    # 1. Load ALL video frame parquets
    ...
    # 3. Combine: oversample video data
    ...
    if len(all_samples) < 100:
        logger.error(f"Only {len(all_samples)} samples!")
        sys.exit(1)
    ...
    # Soft balance: cap majority class at 1.5x minority (more relaxed than v4's 1.15x)
    ...
    # Weight synthetic slightly higher since most errors are synthetic→real
    class_weight = torch.tensor([1.0, 1.2]).to(device)
    criterion = FocalLoss(gamma=2.0, label_smoothing=0.1, weight=class_weight)
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': classifier_params, 'lr': LR * 20},  # Higher ratio for classifier
    ], weight_decay=0.01)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        loss, acc = train_epoch(model, train_loader, optimizer, scheduler,
                                device, epoch, mixup_alpha=0.4, criterion=criterion)
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_model_v5.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_model_v5.pt', weights_only=True))
