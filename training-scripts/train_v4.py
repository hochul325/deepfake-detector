#!/usr/bin/env python3
"""Video deepfake detector training script v4.
Key changes from v3:
- Loads video frame parquets (with 'label' column) directly
- Weights video data heavily (primary training signal)
- Uses image data as regularization only (limited)
- Unfreezes more backbone layers (6 blocks instead of 3)
...
class VideoDeepfakeDetector(nn.Module):
    ...
    def _freeze_backbone(self):
        """Freeze backbone except last 6 transformer blocks and norms."""
        for param in self.encoder.parameters():
            param.requires_grad = False
        for name, param in self.encoder.named_parameters():
            # Unfreeze last 6 blocks (6-11) for better fine-tuning
...
def load_video_frame_parquet(parquet_path, max_samples=10000):
    """Load video frame parquets that have 'image' and 'label' columns.
    Returns list of (PIL.Image, label).
    """
    samples = []
    try:
        ...
        if 'label' not in df.columns or 'image' not in df.columns:
            logger.warning(f"Expected 'image' and 'label' columns in {parquet_path}. Found: {list(df.columns)}")
            return samples
        ...
        logger.info(f"  Loaded {count} video frames from {Path(parquet_path).name} ({n_real} real, {n_synth} synth)")
...
def load_image_parquet(parquet_path, label, max_samples=2000):
    """Load image parquets (HF format without label column)."""
    samples = []
    try:
        table = pq.read_table(parquet_path)
        df = table.to_pandas()
        ...
        if img_col is None:
            logger.warning(f"No image column in {parquet_path}")
            return samples
        ...
        logger.info(f"  Loaded {count} images from {Path(parquet_path).name} (label={'real' if label==0 else 'synth'})")
    except Exception as e:
        logger.warning(f"Error loading {parquet_path}: {e}")
    return samples
...
def package_model(model, output_dir):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "clip-vit-video-deepfake-detector-v4"
version: "4.0.0"
modality: "video"
...
def main():
    ...
    EPOCHS = 20
    BATCH_SIZE = 32
    LR = 1e-5  # Lower LR since unfreezing more layers
    VAL_SPLIT = 0.15
    ...
    # ============================================================
    # Load data - VIDEO FRAMES are PRIMARY, images are secondary
    # ============================================================
    ...
    # 1. Load ALL video frame parquets (these have 'label' column)
    ...
    # 2. Load LIMITED image data (for regularization)
    IMAGE_MAX_PER_FILE = 2000  # Limit images since video is primary
    ...
    for f in sorted(data_dir.glob('*.parquet')):
        if f.name.startswith('video_'):
            continue  # Skip video parquets (already loaded)
        name = f.stem.lower()
    ...
    # 3. Combine with video data oversampled
    # Strategy: oversample video data to be at least equal to image data
    all_samples = list(image_samples)  # copy
    if video_samples:
        # Oversample video data to match image data size
        oversample_factor = max(1, len(image_samples) // max(len(video_samples), 1))
        oversample_factor = min(oversample_factor, 10)  # cap at 10x
        oversampled_video = video_samples * oversample_factor
        # Also ensure we have at least as many video samples as image samples
        while len(oversampled_video) < len(image_samples):
            oversampled_video.extend(random.sample(video_samples, min(len(video_samples), len(image_samples) - len(oversampled_video))))
    ...
    logger.info(f"\nTotal after combining: {len(all_samples)} ({n_real} real, {n_synth} synth)")

    # Balance classes
    if n_real > 0 and n_synth > 0:
        min_class = min(n_real, n_synth)
        max_per_class = int(min_class * 1.15)

        real_samples_all = [(img, l) for img, l in all_samples if l == 0]
        synth_samples_all = [(img, l) for img, l in all_samples if l == 1]
        random.shuffle(real_samples_all)
        random.shuffle(synth_samples_all)

        all_samples = real_samples_all[:max_per_class] + synth_samples_all[:max_per_class]
        n_real = sum(1 for _, l in all_samples if l == 0)
        n_synth = sum(1 for _, l in all_samples if l == 1)
        logger.info(f"After balancing: {len(all_samples)} ({n_real} real, {n_synth} synth)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_model_v4.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_model_v4.pt', weights_only=True))
