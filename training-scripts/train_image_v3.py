#!/usr/bin/env python3
"""Image deepfake detector training v3.
Key changes: SFHQ/StyleGAN, gasstation, leonardo data added. More diverse real data.
Old generators (BigGAN, GLIDE, ADM) capped at 2K. Class weight reduced.
"""
...
def resize_for_memory(img, max_side=512):
    """Resize image to save RAM during loading. Training transform will resize to 224."""
    w, h = img.size
    if max(w, h) > max_side:
...
def train_epoch(model, loader, optimizer, scheduler, device, epoch, mixup_alpha=0.3):
    model.train()
    weight = torch.tensor([1.0, 1.0]).to(device)  # Equal weight to reduce FP
...
def package_model(model, output_dir):
    ...
    (output_dir / 'model_config.yaml').write_text("""name: "clip-vit-image-deepfake-detector-v3"
version: "3.0.0"
modality: "image"
...
def main():
    ...
    OUTPUT_DIR = '/root/image_detector_v3'
    SEED = 42
    EPOCHS = 30
    BATCH_SIZE = 64
    LR = 8e-6
    ...
    real_keywords = ['ffhq', 'celeb', 'coco', 'ms_coco', 'lfw', 'afhq', 'open_image',
                     'flickr', 'fashion', 'caltech', 'food', 'wikiart', 'eidon',
                     'megalith']
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if metrics['sn34_score'] > best_sn34:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), '/tmp/best_image_model_v3.pt')
    ...
    model.load_state_dict(torch.load('/tmp/best_image_model_v3.pt', weights_only=True))
