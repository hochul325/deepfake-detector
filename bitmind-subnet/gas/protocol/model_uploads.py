...
def upload_single_modality(
    ...
    if not presigned_result['success']:
        # Handle 409 - file already exists in cloud storage
        if presigned_result['status_code'] == 409:
            print(f"\n⚡ File already exists in cloud storage (HTTP 409)")
            print(f"  Skipping upload and confirmation steps...")

            # Try to extract existing model info from 409 response
            response_data = presigned_result['response']
            existing_r2_key = None
            existing_model_id = None

            if isinstance(response_data, dict) and 'data' in response_data:
                existing_r2_key = response_data['data'].get('r2_key')
                existing_model_id = response_data['data'].get('model_id')

            if existing_r2_key:
                r2_key = existing_r2_key
                print(f"  Using existing R2 key: {r2_key}")
            else:
                # Construct r2_key from known format: {hotkey}/{timestamp_ms}-{filename}
                timestamp_ms = int(time.time() * 1000)
                hotkey_ss58 = wallet.hotkey.ss58_address
                r2_key = f"{hotkey_ss58}/{timestamp_ms}-{filename}"
                print(f"  Constructed R2 key: {r2_key}")

            print(f"✅ Using existing upload for {modality} model")
            return {
                "success": True,
                "modality": modality,
